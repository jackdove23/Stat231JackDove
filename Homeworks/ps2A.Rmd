...
> ANSWER: In contrast to the five columns of babynames2, BabyNarrow only has three columns, meaning it is "narrower" than babynames2. BabyNarrow is simply providing the number of people with a name for each sex, while babynames2 includes a proportion and the year in question. 

```{r} 
BabyNarrow <- babynames %>%
  filter(year %in% 2000:2017) %>%
  group_by(name, sex) %>%
  summarise(
    total = sum(n)
...
> ANSWER: Blair, Elisha, and Kerry were the top three names, meaning that they had the closest amounts of men and women with that same name. 
...
> ANSWER: BabyNarrow2 is a product of BabyWide, which filtered for names with M>10000 and F>10000, eliminating thousands of names. Therefore, it makes sense that BabyNarrow2 has less observations that BabyNarrow. 
