...
mig_easteurp <- graph_from_edgelist(as.matrix(MigrationEastEurp[,1:2])
                                 , directed = TRUE)

mig_easteurp <- set_edge_attr(migration2, "Y2000"
                            , value = MigrationFlowsF$Y2000)

MigrationEastEurp_network <- ggnetwork(mig_easteurp)
view(MigrationEastEurp_network)

ggplot(data = MigrationEastEurp_network
       , aes(x = x, y = y, xend = xend, yend = yend)) +
  geom_edges(arrow=arrow(type="closed", length=unit(6,"pt"))
            , color = "lightgray") +
  geom_nodes() +
