...
path_in <- "~/Desktop/Data Science/Stat231JackDove/Labs"
...
#Check if there are 100 words with 20+ uses
head(word_frequencies, n=100)
```
...
> ANSWER: The tf=idf will decrease the weight of commonly used words, so stop words will have tiny and irrelevant values.
...
> ANSWER: I think of slick as deceiving, or tricky. (ex: The slick CEO stood at a food bank for a photo op while cutting wages for his employees).
...
> ANSWER: Yes, words from the past might have had different sentiments than they do now.

- The lexicons are based on unigrams.  Do you see any disadvantages of basing the sentiment on single words?

> ANSWER: Yes, if Dickinson writes "The flowers are not beautiful today," beautiful will register a great sentiment/score, but Dickinson meant to say something negative.
...
```{r}
words_w_sentiments <- word_frequencies %>%
...
ggplot(data=words_w_sentiments, aes(x = reorder(word,n)
                           , y = n, fill=word
                  )) +
  geom_col(show.legend = FALSE) +
  labs(x = NULL, y = "Number of Uses") +
  facet_wrap(~ sentiment, ncol = 5, scales = "free") + coord_flip()
...
> ANSWER: I would multiply word frequency by AFINN sentiment score, then for each poem find the median sentiment score, then for the entire corpus find the median sentiment score. 
